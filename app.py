...
if uploaded_file:
    ...
    if not all(col in data.columns for col in required_cols):
        ...
        # ==============================
        # 15. إنذارات الري المبسطة
        # ==============================
        low_threshold = 0.3
        mid_threshold = 0.6

        low_count = (ch_agg["Predicted_Ir"] < low_threshold).sum()
        mid_count = ((ch_agg["Predicted_Ir"] >= low_threshold) & (ch_agg["Predicted_Ir"] < mid_threshold)).sum()
        good_count = (ch_agg["Predicted_Ir"] >= mid_threshold).sum()

        st.subheader("⚠️ إنذارات الري المبسطة")
        st.write(f"🚨 CHs تحتاج ري عاجل: **{low_count}**")
        st.write(f"⚠️ CHs رطوبتها متوسطة: **{mid_count}**")
        st.write(f"✅ CHs رطوبتها جيدة: **{good_count}**")

        # ==============================
        # 16. تقدير استهلاك الماء
        # ==============================
        water_factor = 10  # لتر لكل وحدة Ir
        total_water = ch_agg["Predicted_Ir"].sum() * water_factor

        st.subheader("💧 تقدير استهلاك الماء")
        st.write(f"🔹 كمية المياه المطلوبة لهذه الجولة ≈ **{total_water:.2f} لتر**")

        # ==============================
        # 17. تنبيهات الري التفصيلية
        # ==============================
